...
<div class="ar_meta_control">
  ...
  <div class="greyWhite">&nbsp;</div>
    ...
    <p class="contentSource">
        <strong>Target Audience Segment</strong>
        ...
        foreach ($audienceList->sources as $sourceKey => $source) {
          ...
          foreach ($source->segments as $segmentKey => $segment) {
            ...
            if ($segment->isPrimary) echo "<option value='{$segmentId}' {$selected}>* {$sourceName} - {$segmentName}</option>";
          }
        }
  ...
  <div id="ar-content-results">
    ...
    <div class="resultsArea">
         <div class="resultsAreaInner">
            <div id="ar_score">
              <div class="bar-shadow">
                <div class="bar">
                  <div class="scoreText">
                    Content Score:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span><?php echo round(number_format($aBarColorText['scoring'], 1)); ?></span>
                  </div>
                </div>
              </div>
              <div class="bar-shadow">
        ...
        <ul class="ar_spritelist" id="accordion">
          ...
          <li class="<?= $class;?>"><div>
             <?php 
                  if (strtolower($scoring->data->analysis->tm->state) !== 'green'): ?>
                  ...
                  <?php else: ?>
                  <?=$scoring->data->analysis->tm->message;?></div>
                <?php endif; ?>
          </li>
            <!-- Audience Match -->
            ...
            <?php else:?>  
              ...
              <li class="<?= $class;?>"><div>
                  <?php 
                  if (strtolower($scoring->data->analysis->so->state) !== 'green'): ?>
                  <a href="#so"><?=$scoring->data->analysis->so->message;?></a></div>
                  <ul class="details paragraph-sophistication">
                       <?php 
                       $ul_so = '';
			$ul_so .= '<li style="background: none;"><p class="ar-message"><strong>If your content is too complex,</strong> consider writing shorter sentences, breaking up long paragraphs and/ or using simpler language.<br /><strong>If your content is too simple,</strong> consider elaborating on short sentences and/ or use more sophisticated language.
			<br /><br />
	<a href="https://www.slideshare.net/secret/ydYaIIq7pSJDxK" target="_blank" class="toggle-link ar-message-link">Refer to our tips on how to achieve a content sophistication match for your audience <span>here</span></a>.</p></li>';
                  ...
                  <?php else: ?>
                  <?=$scoring->data->analysis->so->message;?></div>
                <?php endif; ?>
              </li>
            <?php endif; ?>
            ...
            <?php if($scoring->data->analysis->lc->invalid > 0): ?>
                <div>
                  <a href="#lc"><?php echo $scoring->data->analysis->lc->invalid . ' of the ' . $scoring->data->analysis->lc->total . ' links included in the post<br /> are underperforming.'; ?></a>
                </div>
                <div>
            ...
            <?php 
              ...
              switch ($state) {
                case 'green': 
                  echo '<li class="ar_checkmarks_passed"><div>Length of post is ' . $ms . ' sentences, which meets the minimum recommended length of ' . $rs . ' sentences.</div></li>';
                  break;
                case 'yellow':
                  echo '<li class="ar_checkmarks_warning"><div>Length of post is ' . $ms . ' sentences, which is less than the minimum recommended length of ' .$rs . ' sentences.</div></li>';
                  break; 
                case 'red':
                  echo '<li class="ar_checkmarks_error"><div>Length of post is ' . $ms . ' sentences, which is less than the minimum recommended length of ' .$rs . ' sentences.</div></li>';
                  break;
              }     
             ?>
            ...
            <?php if($total_topics > 0): ?>
                <div><a href="#tg"><?php echo $total_topics;?> Topic<?php echo ($total_topics==1) ? '' : 's';?> Identified</a></div>
                <div>
                    <ul class="details">
                       <?php 
                       $ul_tg = '';
